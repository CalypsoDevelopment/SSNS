...
import {SSNS} from "../registry/SSNS.sol";
    ...
    ReverseClaimer
{
    ...
    ) ReverseClaimer(_ssns, msg.sender) {
        ssns = _ssns;
    ...
    function setUpgradeContract(
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            ssns.setApprovalForAll(address(upgradeContract), false);
        }
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            ssns.setApprovalForAll(address(upgradeContract), true);
        }
    }
    ...
    function wrapETH2LD(
        ...
        // transfer the ssns record back to the new owner (this contract)
     ...
     * @param resolver The resolver address to set on the SSNS registry (optional).
     * @param ownerControlledFuses Initial owner-controlled fuses to set
    ...
    function wrap(
        ...
        if (owner != msg.sender && !ssns.isApprovedForAll(owner, msg.sender)) {
            revert Unauthorised(node, msg.sender);
        }

        if (resolver != address(0)) {
            ssns.setResolver(node, resolver);
        }

        ssns.setOwner(node, address(this));
    ...
    function setChildFuses(
        ...
        if (owner == address(0) || ssns.owner(node) != address(this)) {
            revert NameIsNotWrapped();
        }
        // max expiry is set to the expiry of the parent
    ...
    function setSubnodeOwner(
        ...
        if (!_isWrapped(node)) {
            ssns.setSubnodeOwner(parentNode, labelhash, address(this));
    ...
    /*
     * @notice Sets records for the name in the SSNS Registry
    ...
    {
        ssns.setRecord(node, address(this), resolver, ttl);
        if (owner == address(0)) {
