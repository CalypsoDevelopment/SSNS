...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const registry = await ethers.getContract('SSNSRegistry', owner)
  ...
  const providerWithSsns = new ethers.providers.StaticJsonRpcProvider(
    ethers.provider.connection.url,
    { ...ethers.provider.network, ssnsAddress: registry.address },
  )
  const resolver = await providerWithSsns.getResolver('eth')
  if (resolver === null) {
    console.log(
