...
import "../registry/SSNSRegistry.sol";
...
/*
 * @dev An SSNS registrar that allows the owner of a DNS name to claim the
 *      corresponding name in SSNS.
 */
contract DNSRegistrar is IDNSRegistrar, IERC165 {
    ...
    /*
     * @dev This contract's owner-only functions can be invoked by the owner of the SSNS root.
     */
    modifier onlyOwner() {
        Root root = Root(ssns.owner(bytes32(0)));
    ...
    function proveAndClaim(
        ...
        ssns.setSubnodeOwner(rootNode, labelHash, addr);
    }
    ...
    function proveAndClaimWithResolver(
        ...
        ssns.setSubnodeRecord(rootNode, labelHash, owner, resolver, 0);
        if (addr != address(0)) {
